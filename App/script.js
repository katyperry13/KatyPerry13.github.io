...
function generateBars(n = -1) {
    ...
    for (let i = 0; i < n; i++) {
        let barHeight = Math.floor(2 + Math.random() * 98);
        bars.push('<div class="bar" id="' + i + '" style="height:' + barHeight + '%"><div class="bar-value">' + barHeight + '</div></div>');
    }
...
function updateTimeComplexity(complexity) {
    document.getElementById('timeComplexity').innerText = '时间复杂度: ' + complexity;
}
...
function toggleBarValues() {
    let barValues = document.getElementsByClassName('bar-value');
    for(let i = 0; i < barValues.length; i++) {
        barValues[i].classList.toggle('hidden');
    }
}
...
// SelectionSort() : 选择排序算法的实现。时间复杂度为 O(n^2) 
async function SelectionSort() {
    let delay = Disable_The_Input();
    comparisonCount = 0; // 重置比较次数
    ...
    Finished_Sorting();
    // 更新页面上显示比较次数的元素
    document.getElementById('comparisonCount').innerText = '比较次数: ' + comparisonCount;
    updateTimeComplexity('O(n^2)');
}
...
// BubbleSort() : 冒泡排序算法的实现。时间复杂度为 O(n^2)
...
// InsertionSort() : 插入排序算法的实现。时间复杂度为 O(n^2)
...
// 4
// 归并排序
// Slide_down() : 通过将其他bars向右滑动，将 bars[r] 放置在第 l 个位置。. 
...
async function MergeSort() {
    ...
    // 更新页面上显示比较次数的元素
    document.getElementById('comparisonCount').innerText = '比较次数: ' + comparisonCount;
    updateTimeComplexity('平均情况: O(n log n)');
}
...
async function QuickSort() {
    ...
    // 更新页面上显示比较次数的元素
    document.getElementById('comparisonCount').innerText = '比较次数: ' + comparisonCount;
    updateTimeComplexity('平均情况: O(n log n), 最坏情况: O(n^2)');
}
...
async function Heapfiy(n, i, d) {
    ...
    if (l < n) {
        comparisonCount++; // 增加比较次数
        if (parseInt(bars[l].split(/[:%]/)[1]) > parseInt(bars[largest].split(/[:%]/)[1])) largest = l;
    }
    if (r < n) {
        comparisonCount++; // 增加比较次数
        if (parseInt(bars[r].split(/[:%]/)[1]) > parseInt(bars[largest].split(/[:%]/)[1])) largest = r;
    }
